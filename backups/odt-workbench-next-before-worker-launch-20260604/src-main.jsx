...
function useWorkbenchData() {
  ...
  async function refresh() {
    setLoading(true);
    try {
      const [snapshotResult, settingsResult, usageResult, runsResult, standardsResult, evidenceResult, uploadPolicyResult, agentFoundryResult] = await Promise.all([
...
function AgentTeamPage({ setActivePage, data }) {
  ...
  return (
    <div className="page-stack">
      <PageHeader
        eyebrow="Agent Team"
        title="Supervised multi-agent handoff"
        copy="Select Codex, Cline, or Manual execution and prepare a governed handoff. Read-only handoffs are allowed for planning; write-approved handoffs stay locked until standards and approval evidence are current."
