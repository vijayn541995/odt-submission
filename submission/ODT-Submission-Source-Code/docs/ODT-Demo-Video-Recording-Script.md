...
- attached design input: Employee Finder mockup with search, department filter, filtered count, Apply Filters, and Clear Filters controls
...
- show that a mockup image is attached for the same work item
...
`For the feature request, ODT turns a simple work item, its acceptance criteria, and the attached mockup into structured planning. It hardens the requirement, finds the likely impact in the repository, and prepares the work so the next step is clear and reviewable.`
