...
## 0) Preferred Repo-Local Setup

Use the repo-local helper pattern so your OCI settings stay outside committed source:

1. Copy `scripts/odt-oci-local.example.sh` to `scripts/odt-oci-local.sh`
2. Update only your local values in that copied file
3. Source it in the shell where you want to run ODT
...
The source-code upload should include the example file, not a real secret-bearing local file.
