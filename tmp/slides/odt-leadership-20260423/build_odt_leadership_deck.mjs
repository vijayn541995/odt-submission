...
const W = 1280;
const H = 720;
const DECK_ID = 'odt-leadership-20260423';
const OUT_DIR = '/Users/vn105957/Desktop/odt-submission/docs/outputs/2026-04-23-odt-leadership/outputs';
const DECK_FILENAME = 'Oracle-Developer-Twin-Leadership-Deck.pptx';
...
function addChrome(slide, idx, total, dark = false, label = 'LEADERSHIP STORY') {
  const ink = dark ? C.white80 : C.slate;
  const rule = dark ? C.darkLine : C.lightLine;
  textBox(slide, label, 78, 40, 280, 20, { size: 11, color: ink, face: FONT.mono, bold: true, letterSpacing: 1.2 });
  textBox(slide, `${String(idx).padStart(2, '0')} / ${String(total).padStart(2, '0')}`, 1090, 40, 110, 20, { size: 11, color: ink, face: FONT.mono, align: 'right', bold: true, letterSpacing: 1.2 });
  shape(slide, 'rect', 78, 67, 1124, 1.5, rule, C.black0, 0);
}

function addLightBackground(slide, accent = C.blush) {
  slide.background.fill = C.paper;
  shape(slide, 'ellipse', 918, -96, 430, 430, accent, C.black0, 0);
  shape(slide, 'ellipse', -100, 520, 270, 270, '#EBDCC7', C.black0, 0);
  shape(slide, 'ellipse', 1018, 528, 180, 180, '#FFFFFF80', C.black0, 0);
  shape(slide, 'rect', 0, 0, W, H, '#FFFFFF3A', C.black0, 0);
}

function addDarkBackground(slide) {
  slide.background.fill = C.night;
  shape(slide, 'ellipse', 808, -110, 520, 520, '#C7463420', C.redSoft, 1.5);
  shape(slide, 'ellipse', 864, -54, 408, 408, '#FFFFFF05', '#FFFFFF18', 1);
  shape(slide, 'ellipse', 906, -10, 324, 324, C.black0, '#C7463450', 1.5);
  shape(slide, 'ellipse', -120, 520, 320, 320, '#C746341A', '#C7463422', 1);
  shape(slide, 'rect', 920, 0, 180, H, '#C7463418', C.black0, 0);
  shape(slide, 'rect', 980, 0, 26, H, '#FFFFFF05', C.black0, 0);
  shape(slide, 'rect', 1112, 0, 12, H, '#C7463422', C.black0, 0);
}
...
function featureCard(slide, x, y, w, h, title, body, opts = {}) {
  ...
  if (dark) {
    shape(slide, 'ellipse', x + w - 58, y + 18, 22, 22, '#FFFFFF12', C.black0, 0);
  }
}
...
function stageCapsule(slide, x, y, w, h, label, body, accent) {
  ...
  shape(slide, 'rect', x, y, w, 7, accent, C.black0, 0);
  textBox(slide, label, x + 20, y + 18, w - 40, 20, { size: 14, color: C.redDeep, face: FONT.mono, bold: true, letterSpacing: 0.8 });
  textBox(slide, body, x + 20, y + 48, w - 40, h - 60, { size: 16, color: C.charcoal, face: FONT.body });
}

function techBadge(slide, x, y, w, label, fill = '#FFFFFFCA') {
  shape(slide, 'roundRect', x, y, w, 32, fill, C.black0, 0);
  textBox(slide, label, x + 14, y + 8, w - 28, 16, { size: 12, color: C.charcoal, face: FONT.mono, bold: true, letterSpacing: 0.5, valign: 'middle' });
}
...
function quadrantCard(slide, x, y, w, h, title, body, accent, fill) {
  ...
  shape(slide, 'rect', x, y, 8, h, accent, C.black0, 0);
  textBox(slide, title, x + 24, y + 18, w - 40, 24, { size: 16, color: C.ink, face: FONT.mono, bold: true, letterSpacing: 0.7 });
...
function robotMark(slide, x, y, size, dark = false) {
  const headFill = dark ? '#FFFFFF12' : '#FFF7F4';
  const headLine = dark ? '#FFFFFF26' : C.red;
  const eye = dark ? C.white : C.redDeep;
  const mouth = dark ? '#FFFFFFCC' : C.charcoal;
  const accent = dark ? C.redSoft : C.red;
  shape(slide, 'ellipse', x + size * 0.22, y, size * 0.16, size * 0.16, accent, C.black0, 0);
  shape(slide, 'rect', x + size * 0.29, y + size * 0.10, size * 0.02, size * 0.11, accent, C.black0, 0);
  shape(slide, 'roundRect', x, y + size * 0.18, size * 0.60, size * 0.46, headFill, headLine, 1.4);
  shape(slide, 'ellipse', x + size * 0.13, y + size * 0.34, size * 0.10, size * 0.10, eye, C.black0, 0);
  shape(slide, 'ellipse', x + size * 0.37, y + size * 0.34, size * 0.10, size * 0.10, eye, C.black0, 0);
  shape(slide, 'roundRect', x + size * 0.17, y + size * 0.49, size * 0.26, size * 0.07, mouth, C.black0, 0);
  shape(slide, 'rect', x - size * 0.08, y + size * 0.32, size * 0.08, size * 0.04, accent, C.black0, 0);
  shape(slide, 'rect', x + size * 0.60, y + size * 0.32, size * 0.08, size * 0.04, accent, C.black0, 0);
}
...
function dashboardMock(slide, x, y, w, h) {
  ...
  textBox(slide, 'Oracle-themed Digital Worker Console', x + 32, y + 34, 290, 16, { size: 13, color: C.white80, face: FONT.mono, bold: true, letterSpacing: 0.7 });
...
function slide9(presentation) {
  ...
  shape(slide, 'ellipse', 474, 252, 332, 332, '#F2E3DB', C.redSoft, 1.2);
  shape(slide, 'ellipse', 522, 300, 236, 236, '#FFF7F4', C.lightLine, 1);
  textBox(slide, 'Reusable\nOracle pattern', 548, 360, 184, 74, { size: 26, color: C.redDeep, face: FONT.title, bold: true, align: 'center', valign: 'middle' });
  textBox(slide, 'Governable AI\ndelivery', 556, 442, 168, 40, { size: 16, color: C.charcoal, face: FONT.body, align: 'center' });
